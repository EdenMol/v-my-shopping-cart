export default {
    ...
    getters: {
        ...
        totalPrice: (state) =>
            state.cart.reduce((total, cartItem) => total + cartItem.quantity * cartItem.product.price, 0)
    },
    mutations: {
        ...
        subtractProduct(state, id) {
            ...
            if (cartItem.quantity == 1) {
                const index = state.cart.findIndex(cartItem => cartItem.product._id == id)
                if (index != -1) {
    ...
    actions: {
        ...
        initialiazeCart(context, store) {
            ...
            //watcher
            store.watch(state => state.cart.cart, () => context.dispatch("storeCartData"), { deep: true })
        },
